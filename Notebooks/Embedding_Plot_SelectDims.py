...
Xdim_select = pn.widgets.IntSlider(name='X Dimension',value=1,start=1, end=20, step=1)
Ydim_select = pn.widgets.IntSlider(name='Y Dimension',value=2,start=1, end=20, step=1)
Zdim_select = pn.widgets.IntSlider(name='Z Dimension',value=3,start=1, end=20, step=1)
...
def plot_embed3d(max_win,SBJ,RUN,WL_sec,COLOR,x_dim,y_dim,z_dim):
    ...
    LE3D_df = LE3D_df[['x'+str(x_dim).zfill(2),'x'+str(y_dim).zfill(2),'x'+str(z_dim).zfill(2),
                      'x'+str(x_dim).zfill(2)+'_norm','x'+str(y_dim).zfill(2)+'_norm','x'+str(z_dim).zfill(2)+'_norm',
                      'Run','Sleep Value','Sleep Stage','mean FD','label']]
    LE3D_df.columns=['x','y','z','x_norm','y_norm','z_norm','Run','Sleep Value','Sleep Stage','mean FD','label']
...
@pn.depends(SubjSelect.param.value,RunSelect.param.value,WindowSelect.param.value,
            Xdim_select.param.value,Ydim_select.param.value,Zdim_select.param.value)
def dist_mot_trace(SBJ,RUN,WL_sec,x_dim,y_dim,z_dim):
    """
    Plot the distance matrix, motion trace, and sleep stage trace in line with each other
    """
    output = (distance_matrix(SBJ,RUN,WL_sec,x_dim,y_dim,z_dim)+motion_trace(SBJ,RUN,WL_sec)+sleep_stage_trace(SBJ,RUN,WL_sec)).cols(1)
    return output
