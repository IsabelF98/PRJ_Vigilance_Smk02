...
num_tp,aux = EEG_sbj_sleep_df.shape # Size of subject sleep data frame
max_tp = int(EEG_sbj_sleep_df.loc[int(num_tp)-1]['TR']) # Minimum number TR in data
...
if min_TR != 0:
    temp_top = pd.DataFrame(columns=['dataset','subject','cond','TR','sleep','drowsiness','spectral','seconds'],index=range(0,min_TR))
    temp_top['dataset'] = EEG_sbj_sleep_df['dataset']
    temp_top['subject'] = EEG_sbj_sleep_df['subject']
    temp_top['cond']    = EEG_sbj_sleep_df['cond']
    temp_top['TR']      =range(0,min_TR)

# Fill in missing TR data bellow
if max_tp != TIME-1:
    temp_bot = pd.DataFrame(columns=['dataset','subject','cond','TR','sleep','drowsiness','spectral','seconds'],index=range(0,TIME-(max_tp+1)))
    temp_bot['dataset'] = EEG_sbj_sleep_df['dataset']
    temp_bot['subject'] = EEG_sbj_sleep_df['subject']
    temp_bot['cond']    = EEG_sbj_sleep_df['cond']
    temp_bot['TR']      =range(max_tp+1,TIME)
