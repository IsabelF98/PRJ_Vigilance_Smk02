...
def sleep_bar_graph(SBJ):
    ...
    count_df = pd.DataFrame(columns=['Run','Sleep_Stage','Count'])
    sleep_list = list(sleep_df['sleep_stage'])
    for stage in ['Wake','Stage 1','Stage 2','Stage 3', 'Undetermined']:
        count_df.loc[len(count_df.index)] = ['Sleep',stage,sleep_list.count(stage)]
    wake_list = list(wake_df['sleep_stage'])
    for stage in ['Wake','Stage 1','Stage 2','Stage 3', 'Undetermined']:
        count_df.loc[len(count_df.index)] = ['Wake',stage,wake_list.count(stage)]
        
    output = hv.Bars(count_df,kdims=['Run','Sleep_Stage']).opts(width=800,height=350,title='Sleep Stage Bar Graph for '+SBJ)
    return output
